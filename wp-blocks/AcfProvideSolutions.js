	...
	return (
		...
		>
			<img
				...
				src={require('../assets/images/hexagon.svg')?.default?.src}
				alt=''
			/>
			<div className='block_content'>
				{title.length > 0 && (
					<div className='pb-[60px] lg:pb-[90px]'>
						<h2
							className='text-center text-[#fff] lg:w-[38%] m-auto'
							dangerouslySetInnerHTML={{ __html: title ?? '' }}
						/>
						<p className='paragraph sm:w-[60%] text-center text-[#929292] pt-[30px] lg:w-[55%] m-auto '>
							{headline}{' '}
						</p>
					</div>
				)}
				<div className='flex flex-row flex-wrap'>
					{cards.map((card, index) => {
						return (
                            <div
								key={index}
								className={`w-full lg:w-1/3 px-[30px] py-[51px]  border-l ${
									background == 'Light'
										? 'border-[#80808061]'
										: 'border-[#404040]'
								}  card-line relative`}
							>
								...
								<p
									className={`${
										background == 'Light' ? 'text-background' : 'text-[#FFF] '
									}  font-[600] text-[16px] pt-[20px]`}
								>
									{' '}
									{card.title}{' '}
								</p>
								<p className='text-[16px] font-[400] text-[#929292] py-[15px]'>
									{' '}
									{card.paragraph}{' '}
								</p>
								<Link
                                    ...
                                    className='text-[#0AADE5] text-[16px] cursor-pointer flex hover-arrow'>
				...
				<div className='flex justify-center'>
					{learnMoreLink?.url && (
						(<Link
                            ...
                            className='button_custom inline-block z-1 mt-[50px] lg:mt-[80px]'
			...
			<img
				loading='lazy'
				className='hidden lg:block absolute z-10 bottom-[-60px] right-0'
				src={require('../assets/images/hexagon-3.svg')?.default?.src}
				alt=''
			/>
		</section>
    );
