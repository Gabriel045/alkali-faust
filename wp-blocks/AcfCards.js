...
export default function AcfCards({data}) {
  ...
  console.log(cards);
    ...
    return (
    <section className="">
      <div className="block_content <?php echo $block['className'] ?>">
          <div className={`flex flex-wrap gap-[56px]  ${numberOfColumns == "Four" ? 'xl:gap-[60px]' : 'xl:gap-[90px] lg:gap-y-[48px]'} `}>
       {  cards.map((card) =>{
            return(
              <div className={`rounded-[20px]  p-[30px] w-full  ${numberOfColumns == "Four" ? 'sm:w-[46%] xl:w-[21%]' : 'sm:w-[45%] xl:w-[28%] '}   ${cardBackground == "Yes" ?? "bg-[#FAFAFA]"} `} >
                <Image
                ...
                />
                 ...
                 <a className="text-primary text-[16px] lg:text-[18px] font-[500] cursor-pointer flex hover-arrow" href="<?php echo $card['link']['url'] ?> "> {card?.link?.text} 
                  <Image
