	...
	return (
        <section>
			<div className={'block_content ' + cssClassNames}>
				<div
					className={`flex flex-wrap gap-[56px]  ${
						numberOfColumns == 'Four'
							? 'xl:gap-[60px]'
							: 'xl:gap-[90px] lg:gap-y-[48px]'
					} `}
				>
					{cards.map(card => {
						return (
                            <div
								className={`rounded-[20px]  p-[30px] w-full  
                ${
									numberOfColumns == 'Four'
										? 'sm:w-[46%] xl:w-[21%]'
										: 'sm:w-[45%] xl:w-[28%] '
                ...
                ${cardBackground == 'Yes' ? 'bg-[#FAFAFA]' : ''} `}
							>
								...
								/>
								<p className='mt-[30px] lg:mt-[53px] mb-[18px] text-[24px] leading-[30px] text-background font-[500]'>
									{' '}
									{card?.title}{' '}
								</p>
								<p className='text-[16px] leading-[24px] text-[#23232399] font-[400] mb-[32px]'>
									{' '}
									{card?.text}{' '}
								</p>
								{card.link?.url && (
									(<Link
                                        ...
                                        className='text-primary text-[16px] lg:text-[18px] font-[500] cursor-pointer flex hover-arrow'>
