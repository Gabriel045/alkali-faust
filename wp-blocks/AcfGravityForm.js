	...
	const { data } = useQuery(GET_FIELDS, {
		variables: {
			formID: props?.data?.gravytyFormBlock?.idForm
				? props?.data?.gravytyFormBlock?.idForm
				: props?.idForm,
		},
	})
		...
		switch (type.type) {
			case 'TEXT':
				return (
					<div
						id={type.id}
						className={`input-content flex flex-col ${
							type.gridColumn == 6 ? 'md:w-[49%] w-full' : 'w-full'
						} `}
					>
						<label className='text-[14px] font-[600] mb-[5px]'>
							{type.label}
						</label>
						<input
							className='font-[300] py-[10px] px-[14px] rounded-[10px] border-[1px] border-[#D0D5DD]'
							placeholder={type.placeholder}
							key={type.label}
							type='text'
							required={`${type.isRequired == true ? 'required' : ''}`}
							{...register(`${type.label}`)}
						/>
					</div>
				)
			...
			case 'POST_CONTENT':
				return (
					...
					>
						...
						<textarea
							className='font-[300] h-[106px] py-[10px] px-[14px] rounded-[10px] border-[1px] border-[#D0D5DD]'
							placeholder={type.placeholder}
							key={type.label}
							required={`${type.isRequired == true ? 'required' : ''}`}
							{...register(`${type.label}`)}
	...
	return (
		<section id='GravyForm'>
				...
				<div className='flex flex-wrap lg:flex-nowrap lg:gap-[45px]'>
					<div
						className={`flex flex-col ${
							props?.idForm ? 'w-full' : 'lg:w-1/2'
						} `}
					>
						{!props?.idForm && (
							<h4 className='text-[18px] lg:text-[24px] text-background font-[600] mb-[40px] lg:mb-[60px]'>
            ...
            `,
							...
							<form
								key='form'
								className='flex flex-wrap gap-y-[20px] gap-x-[8px] bg-[#F9FAFB] p-[46px] rounded-[16px]'
							...
							>
								{isLoading || loading ? (
									...
									<>
										...
										<button
											className='w-full rounded-[8px] px-[20px] py-[10px] text-white text-[16px] font-[500] bg-[#00BAFB]'
											type='submit'
										>
					...
					{!props?.idForm && (
						<div
							className='lg:w-1/2 rounded-[10px] h-[400px] w-full  mt-[50px] lg:h-auto lg:mt-0 min-h-[618px] bg-cover'
							style={style}
						/>
					)}
				</div>
