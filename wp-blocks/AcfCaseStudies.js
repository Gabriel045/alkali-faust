	...
	return (
        <section className='bg-background'>
			<div className='block_content relative'>
				<img
					loading='lazy'
					className='absolute z-10 top-[15%] left-[-5%] rotate-180'
					src={require('../assets/images/hexagon-5.svg')?.default?.src}
					alt=''
				/>
				<div className='pb-[60px] lg:pb-[120px]'>
					<h2
						className='text-center text-[#fff] lg:w-[60%] m-auto'
				...
				<div className='flex flex-row flex-wrap'>
					{cards.map((card, index) => {
						return (
                            <div
								key={index}
								className='lg:w-[48%] px-[44px] py-[46px] rounded-[10px] mb-[35px] lg:mb-0 lg:first:mr-[4%] border-custom z-50'
							>
								...
								<p className='text-[#FFF] font-[600] text-[24px] relative'>
									{' '}
									{card?.title}{' '}
								</p>
								<p className='text-[18px] font-[400] text-[#929292] pt-[23px] pb-[37px] relative'>
									{' '}
									{card?.paragraph}{' '}
								</p>
								{card?.link?.url && (
									(<Link
                                        ...
                                        className='text-[#0AADE5] text-[16px] flex relative hover-arrow'>
				...
				<div className='mt-[60px] lg:mt-[140px] flex justify-center gap-[30px] lg:gap-[20px] flex-wrap lg:flex-nowrap'>
					<Link
                        href={browseCaseStudies?.url ?? '#'}
                        target={browseCaseStudies?.target}
                        className='text-white w-auto py-[15px] px-[35px] bg-[#333232] rounded-[10px] transform hover:translate-y-[2px] border-button'>

                        {' '}
                        {browseCaseStudies?.title}{' '}
