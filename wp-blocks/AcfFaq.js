...
export default function AcfFaq({data}) {
    ...
    return (
        <section id="faq" className="text-block dark ">
                    ...
                    <div className="left w-full lg:w-1/2 border-l-[3px] border-[#7D7D7D] lg:mr-[22px]">
                        {
                            leftCol.map((leftItem, index) => {
                                return (
                                    <div key={index} ref={leftRef.current[index]} className={` step-${index} slider-faq ml-[-3px] mb-[75px] lg:last:mb-0  disable`}>
                                        <h4 className="text-[#FFF] cursor-pointer relative" onClick={() => slide(index)}> {leftItem?.title} </h4>
                                        <div className="item-content">
                                            <p className="paragraph text-[#FFF] pt-[26px] pb-[16px]"> {leftItem?.paragraph} </p>
                                        </div>
                                    </div>
                                )
                    ...
                    <div className="right w-full lg:w-1/2 border-l-[3px] border-[#7D7D7D] lg:ml-[22px]">
                        {
                            rightCol.map((rightItem, index) => {
                                return (
                                    <div key={index} ref={rightRef.current[index]}  className={` step-${index} slider-faq ml-[-3px] mb-[75px] last:mb-0  disable`}>
                                        <h4 className="text-[#FFF] cursor-pointer relative" onClick={() => slideRight(index)}>{rightItem?.title}</h4>
                                        <div className="item-content">
                                            <p className="paragraph text-[#FFF] pt-[26px] pb-[16px]"> {rightItem?.paragraph}</p>
                                        </div>
                                    </div>
                                )
                            })
