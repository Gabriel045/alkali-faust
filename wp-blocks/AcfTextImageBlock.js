...
export default function AcfTextImageBlock({ data }) {
	...
	const image_position = data.textImageBlock?.imagePosition
		? data.textImageBlock?.imagePosition[0]
		: ''
	const image = data.textImageBlock?.image?.node?.sourceUrl
		? data.textImageBlock?.image?.node?.sourceUrl
		: ''
	...
	return (
        <section
			id={customID}
			className={`text-block ${background}`}
		>
			<div
				className={`block_content flex flex-wrap lg:flex-nowrap gap-[80px] flex-${image_position}`}
			>
				<div
					className={`flex flex-wrap lg:flex-nowrap gap-[80px] w-full flex-${image_position} `}
				>
					{image.length > 0 && (
					...
					<div
						className={`${
							image.length > 0 ? 'w-full lg:w-[50%]' : 'full'
						} lg:flex lg:flex-col lg:justify-center relative`}
					>
						<h3 className='z-50 relative'> {title} </h3>
						<div
							className={`paragraph text-[#929292] z-50 relative ${
								title ? 'py-[32px]' : ''
							}`}
						...
						{learnMoreUrl?.url && (
							(<Link
                                href={learnMoreUrl}
                                target={learnMoreUrl?.target}
                                className={`button_custom inline-block ${
                                    background == 'light' ? 'no-border' : ''
                                }`}>
