...
export default function AcfSlider({ data }) {
	...
	const slide = step => {
		refs.current.map(div => {
			...
			if (id == step) {
				div.current.className.includes('enable')
					? ''
					: (div.current.className += ' enable')
			} else {
		...
		>
			<div className='block_content'>
				<div className='pb-[60px] lg:pb-[120px]'>
					<h2
						className='text-center text-[#232323]'
						dangerouslySetInnerHTML={{ __html: title ?? '' }}
					/>
					<p className='paragraph text-center text-[#525252] pt-[30px] sm:w-[60%] lg:w-[55%] m-auto '>
						{headline}{' '}
					</p>
				</div>
							...
							return (
								...
								>
									...
									<div className='item-content'>
										<p className='text-[16px] font-[400] text-[#23242499] pt-[26px] pb-[16px]'>
											{' '}
											{item.paragraph}{' '}
										</p>
										<Link
                                            href={item?.learnMore?.url ?? '#'}
                                            className='text-[#0AADE5] text-[16px] flex hover-arrow'
                                            target={item?.learnMore?.target}>
									...
									<div className='w-[56%] side-image h-[700px] rounded-[10px]  z-50 absolute right-0 top-0 side-image'>
										...
										<div className='flex h-full items-end absolute top-0  px-[50px] xl:px-[50px] sm:px-[20px] py-[50px]'>
											<div className='w-[60%]'>
												<p className='image-text text-white text-[23px] font-[600] leading-[37px]'>
													{item.textImage}{' '}
												</p>
											</div>
